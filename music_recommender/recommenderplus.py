...
def getPreferences(userName, userMap):
	...
	if userName in userMap:
		...
		newPref = raw_input("like, or just press enter to continue: ")
	else:
		prefs = []
...
def mostLikes(userMap):
	...
	for i in range(len(users)):
		if (len(userMap[users[i]]) > longest) and (users[i][len(users[i])-1] != "$"):
			longest = len(userMap[users[i]])
			most_likes = users[i]
	print most_likes + " has the most likes!"
	print "Here are "+most_likes+"'s "+"preferences: " + str(userMap[most_likes])
