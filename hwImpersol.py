...
# You should add code for testing, clearly marked as such.

# To find the incomplete functions, search for "TO DO".

############
# Exercise 0
# Study function questify().  Then implement questifyAlt() so that it 
# gives the same results as questify(), using map and lambda but no helping function.
...
def questifyAlt(strs):
    ''' Same as questify.'''
    return map(lambda item: item+'?', strs)
...
def letterScore(letter, scorelist): 
    ''' Assume scorelist is a list of lists [ltr,val] where ltr is a single letter and val is a
        natural number.  Assume letter is a single letter string, that occurs in scorelist.
...
def letterScoreTest():
    ...
    for ltr in ['a', 'b', 'x']:  
        print "letterScoreTest: test case ", ltr
        assert letterScore(ltr, scrabbleScores) == letterScoreLoop(ltr, scrabbleScores)
...
def wordScoreLoop(S, scorelist): 
	...
	if S=='':
		...
		for i in range(len(S)):
			x += letterScoreLoop(S[i],scorelist)
		return x

def wordScoreTest():
    ''' Test consistency between two versions of wordScore.'''
    for word in ['ok', 'brilliant', 'splendid']:  
        print "wordScoreTest: testing case ", word
        assert wordScore(word, scrabbleScores) == wordScoreLoop(word, scrabbleScores)
...
def wordsWithScoreLambda(dict, scores):
	...
	return map(lambda item: [item,wordScoreLoop(item,scores)],dict)
...
def wordsWithScoreLoop(dict, scores):
	...
	for i in range(len(dict)):
		list = list + [[dict[i],wordScoreLoop(dict[i],scores)]]
	return list
...
def wordsWithScoreTest():
    ...
    print 'wordsWihScoreTest:'
    print 'Lambda version, on aDictionary'
    assert M == L
    print 'Loop version, on aDictionary'
    assert N == L
